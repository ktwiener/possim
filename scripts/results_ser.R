...
measures %>%
  dplyr::filter(grepl("lnrr", Param)) %>%
  dplyr::filter(positivity == "Full") %>%
boxplot() +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=0.5))


 ggsave(filename = sprintf("data/results/figures/%s-homogeneous-hajek-box-complete.png", date),
       units = "in",
       width = 6.5,
       height = 5.6
)
 measures %>%
   dplyr::filter(grepl("lnrr", Param)) %>%
   dplyr::filter(positivity == "Partial") %>%
   boxplot()+
   theme(panel.border = element_rect(colour = "black", fill=NA, size=0.5))


 ggsave(filename = sprintf("data/results/figures/%s-homogeneous-hajek-box-partial.png", date),
        units = "in",
        width = 6.5,
        height = 5.6
 )
